...
public class LoginController extends BaseController{
	 ...
	 * @return
	 */
	@RequestMapping(params = "andrew")
	public ModelAndView andrew(){
		return new ModelAndView("login/andrew");
	}
	...
	public AjaxJson checkuser(TSUser user, HttpServletRequest req) {
		...
		Map<String,String> p = ParameterUtil.getParamMaps(req.getParameterMap());
		...
		if (StringUtils.isEmpty(randCode)) {
			...
			//用户登录验证逻辑
			Map checkUserMap = this.systemService.findOneForJdbc("select t1.*,t2.username from t_s_user t1 left join t_s_base_user t2 on t2.id=t1.id where (t2.username=? or t1.mobilePhone=?)",user.getUserName(),user.getUserName());
			if(checkUserMap == null){
				j.setMsg("您输入的用户名有误，请重新输入！");
				j.setSuccess(false);
				return j;
			}else{
				user.setUserName(checkUserMap.get("username").toString());
			}
			...
			if (u != null && u.getStatus() != 0) {
				...
				/*if(Utils.isEmpty(req.getSession().getAttribute("ipText"))){
					String ip = Utils.getNetIp();
					req.getSession().setAttribute("ipText",ip);
				}*/
			...
			Map roleMap = systemService.findOneForJdbc("SELECT GROUP_CONCAT(t2.rolename) rolename,GROUP_CONCAT(t2.rolecode) rolecode FROM t_s_role_user t1 LEFT JOIN t_s_role t2 ON t1.`roleid`=t2.`ID` where userid=?",u.getId());
	...
	public String login(ModelMap modelMap,HttpServletRequest request,HttpServletResponse response) {
		...
		if(Utils.notEmpty(param.get("showWin"))){
