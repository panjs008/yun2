...
public class SystemController extends BaseController {
	...
	public void typeGroupGrid(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid, TSTypegroup typegroup) {
		...
		cq.eq("orgCode",user.getCurrentDepart().getOrgCode());
		...
		HqlGenerateUtil.installHql(cq, typegroup, request.getParameterMap());
...
//    }
	...
	public AjaxJson delTypeGroup(TSTypegroup typegroup, HttpServletRequest request) {
		...
		systemService.executeSql("delete from t_s_type where typegroupid=?",typegroup.getId());
		systemService.delete(typegroup);
		systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
		//刷新缓存
		systemService.refleshTypeGroupCach();
	...
	@RequestMapping(params = "delBatchType")
	@ResponseBody
	public AjaxJson delBatchType(String ids,TSType type, HttpServletRequest request) {
		String message = "删除成功";
		AjaxJson j = new AjaxJson();
		try {
			for(String id:ids.split(",")){
				type = systemService.getEntity(TSType.class, id);
				systemService.delete(type);
				//刷新缓存
				systemService.refleshTypesCach(type);
			}
	...
	public AjaxJson saveTypeGroup(TSTypegroup typegroup, HttpServletRequest request) {
		...
		typegroup.setOrgCode(tsDepart.getOrgCode());
		typegroup.setDepartId(tsDepart.getId());
	...
	public AjaxJson saveType(TSType type, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(type.getId())) {
			...
			TSTypegroup tsTypegroup = systemService.get(TSTypegroup.class,type.getTSTypegroup().getId());
			if("zjzh".equals(tsTypegroup.getTypegroupcode())){
				...
				emkBankRecordEntity.setOrgCode(tsDepart.getOrgCode());
				emkBankRecordEntity.setDepartId(tsDepart.getId());
				emkBankRecordEntity.setDealDate(DateUtil.getCurrentTimeString(null));
				emkBankRecordEntity.setRemark("期初余额");
				emkBankRecordEntity.setStorageId(user.getStorageId());
				emkBankRecordEntity.setStorageName(user.getStorageName());
				emkBankRecordEntity.setBalance(param.get("money"));
				emkBankRecordEntity.setIncome(param.get("money"));
				emkBankRecordEntity.setOutcome("");
				emkBankRecordEntity.setBankType(type.getTypecode());
				emkBankRecordEntity.setBankAccount(type.getTypename());
...
//			depart.setOrgCode(orgCode);
			if(oConvertUtils.isNotEmpty(pid)){
				...
				depart.setParentOrgCode(paretDept.getOrgCode());
			}else{
