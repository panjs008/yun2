package com.emk.caiwu.yfcheck.service.impl;
...
@Service("emkFinanceYfCheckService")
@Transactional
public class EmkFinanceYfCheckServiceImpl extends CommonServiceImpl implements EmkFinanceYfCheckServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(EmkFinanceYfCheckEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(EmkFinanceYfCheckEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(EmkFinanceYfCheckEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(EmkFinanceYfCheckEntity t){
		...
		map.put("yf_check_no", t.getYfCheckNo());
		map.put("yftz_date", t.getYftzDate());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,EmkFinanceYfCheckEntity t){
 		...
 		sql  = sql.replace("#{yf_check_no}",String.valueOf(t.getYfCheckNo()));
 		sql  = sql.replace("#{yftz_date}",String.valueOf(t.getYftzDate()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("emk_finance_yf_check",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
