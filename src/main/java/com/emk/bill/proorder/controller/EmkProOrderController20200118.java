...
public class EmkProOrderController20200118 extends BaseController {
    private static final Logger logger = Logger.getLogger(EmkProOrderController20200118.class);
    ...
    public ModelAndView orderMxList(HttpServletRequest request) {
        Map map = ParameterUtil.getParamMaps(request.getParameterMap());
        TSUser user = (TSUser) request.getSession().getAttribute(ResourceUtil.LOCAL_CLINET_USER);
        List<Map<String, Object>> categoryEntities = this.systemService.findForJdbc("select lower(code) code,queryed,name,lower(PARENT_CODE) parentCode,required,column_type from hm_category where PARENT_CODE='A01A09' and code!='A01A09A05' and org_code=? and is_show=0 order by order_num asc",user.getCurrentDepart().getOrgCode().substring(0,3));
        request.setAttribute("categoryEntities",categoryEntities);
        List<Map<String, Object>> ordercategoryEntities = this.systemService.findForJdbc("select lower(code) code,queryed,name,lower(PARENT_CODE) parentCode,required,column_type from hm_category where PARENT_CODE='A01A06' and org_code=? and is_show=0 order by order_num asc",user.getCurrentDepart().getOrgCode().substring(0,3));
        request.setAttribute("ordercategoryEntities",ordercategoryEntities);
        String columns = "";
    ...
    public AjaxJson doAdd(EmkProOrderEntity emkProOrder,EmkSizeEntity emkSize, HttpServletRequest request) {
        ...
        try {
            ...
            List<Map<String, Object>> categoryEntities = this.systemService.findForJdbc("select lower(code) code,queryed,name,lower(PARENT_CODE) parentCode,required,column_type from hm_category where PARENT_CODE='A01A06' and org_code=? and is_show=0 order by order_num asc",orgCode);
            ...
            Class c2 = Class.forName(EmkMInStorageDetailEntity.class.getName());

            List<Map<String, Object>> commoncategoryEntities = this.systemService.findForJdbc("select lower(code) code,queryed,name,lower(PARENT_CODE) parentCode,required,column_type from hm_category where PARENT_CODE='A01A09' and org_code=? and is_show=0 order by order_num asc",orgCode);
            ...
            if (Utils.notEmpty(dataRows)) {
                ...
                for (int i = 0; i < rows; i++) {
                    if(Utils.notEmpty(map.get("orderMxList["+i+"].a01a09a07"))) {
                        ...
                        emkMInStorageDetailEntity.setDepartId(emkProOrder.getDepartId());
                        emkMInStorageDetailEntity.setOrgCode(emkProOrder.getOrgCode());
                        emkMInStorageDetailEntity.setpId(emkProOrderDetailEntity.getId());
                        for (Map category : commoncategoryEntities) {
                            if (Utils.notEmpty(map.get("orderMxList[" + i + "]." + category.get("code")))) {
                                String m0 = "setA" + category.get("code").toString().substring(1);
                                show = c2.getMethod(m0, String.class);
    ...
    public AjaxJson doUpdate(EmkProOrderEntity emkProOrder,EmkSizeEntity emkSize, HttpServletRequest request) {
        ...
        try {
            ...
            if (Utils.notEmpty(dataRows)) {
                int rows = Integer.parseInt(dataRows);
                systemService.executeSql("delete from emk_m_in_storage_detail where FIND_IN_SET(p_id,(SELECT GROUP_CONCAT(id) FROM emk_pro_order_detail where PRO_ORDER_ID=?))", t.getId());
