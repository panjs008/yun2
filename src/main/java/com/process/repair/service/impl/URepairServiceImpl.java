package com.process.repair.service.impl;
...
@Service("uRepairService")
@Transactional
public class URepairServiceImpl extends CommonServiceImpl implements URepairServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(URepairEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(URepairEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(URepairEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(URepairEntity t){
		...
		map.put("equip_type", t.getEquipType());
		map.put("brand", t.getBrand());
		map.put("fault", t.getFault());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,URepairEntity t){
 		...
 		sql  = sql.replace("#{equip_type}",String.valueOf(t.getEquipType()));
 		sql  = sql.replace("#{brand}",String.valueOf(t.getBrand()));
 		sql  = sql.replace("#{fault}",String.valueOf(t.getFault()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("u_repair",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
