...
<script type="text/javascript">
       ...
       $('#orgtree').tree({
            ...
            onClick: function (node) {
                if ($('#orgtree').tree('isLeaf', node.target) || $('#orgtree').tree('expand', node.target)) {
                    $('#hmStaffListForBc').datagrid("reload", {"orgCode": node.id});
    ...
    function gridLoadSuccess (data) {
        var rowsData = $('#hmStaffListForBc').datagrid('getData');
        $.ajax({
                ...
                for ( var i = 0; i < rowsData.rows.length; i++) {
                    for(var id in staffSelectArr){
                        if(rowsData.rows[i].gdNamesCode == staffSelectArr[id]){
                            $('#hmStaffListForBc').datagrid('checkRow',i);
...
<div class="easyui-layout" fit="true">
 <div region="west" style="width: 180px;" title="组织机构" split="true">
  ...
  <div region="center" style="padding:0px;border:0px">
      <t:datagrid name="hmStaffListForBc" onLoadSuccess="gridLoadSuccess" onClick="doCancelSession" checkbox="true" pagination="true" fitColumns="false" title="" actionUrl="hmStaffController.do?datagridC&state=0" idField="id" fit="true" btnCls="bootstrap" queryMode="group">
   <t:dgCol title="主键"  field="id"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="创建人名称"  field="createName"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="创建人登录名称"  field="createBy"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="创建日期"  field="createDate"  formatter="yyyy-MM-dd"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="所属部门"  field="sysOrgCode"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
          <t:dgCol title="工号"  field="gdNamesCode"  queryMode="single"  width="70"></t:dgCol>
          <t:dgCol title="姓名"  query="true"  field="gdNames"  queryMode="single"  width="80"></t:dgCol>
          <t:dgCol title="部门"   field="deptName" query="true" queryMode="single"  width="100"></t:dgCol>
          ...
          <t:dgCol title="级别"  field="jb" query="true"  dictionary="jb" queryMode="single"  width="70"></t:dgCol>
          ...
          <t:dgCol title="员工类别" query="true"  field="yglb" dictionary="yglb" queryMode="single"  width="80"></t:dgCol>
 ...
 function saveSession() {
     var rowsData = $('#hmStaffListForBc').datagrid('getSelections');
     var ids = [];
