...
<script type="text/javascript">
       ...
       $('#orgtree').tree({
            ...
            onClick: function (node) {
                if ($('#orgtree').tree('isLeaf', node.target) || $('#orgtree').tree('expand', node.target)) {
                    $('#hmStaffList3').datagrid("reload", {"orgCode": node.id});
    ...
    function gridLoadSuccess (data) {
        var rowsData = $('#hmStaffList3').datagrid('getData');
        $.ajax({
            url : "hmStaffController.do?getSessionStaff&r=lsJdNamesCode",
            type : 'post',
            cache : false,
            success : function(res) {
                ...
                for ( var i = 0; i < rowsData.rows.length; i++) {
                    for(var id in staffSelectArr){
                        if(rowsData.rows[i].lsJdNamesCodeCode == staffSelectArr[id]){
                            $('#hmStaffList3').datagrid('checkRow',i);
                            checkids.push(staffSelectArr[id]);
                        }
                    }
                }
            }
        });
       /* var staffSelectArr = '${sessionScope.lsJdNamesCodeS}'.split(",");
        for ( var i = 0; i < rowsData.rows.length; i++) {
            for(var id in staffSelectArr){
                if(rowsData.rows[i].lsJdNamesCodeCode == staffSelectArr[id]){
                    $('#hmStaffList3').datagrid('checkRow',i);
    ...
    function doCancelSession(data){
        var rowsData = $('#hmStaffList3').datagrid('getData');
        $.ajax({
            url : "hmStaffController.do?doCancelSession&r=lsJdNamesCodeS&workNo="+rowsData.rows[data].lsJdNamesCode,
            type : 'post',
            cache : false,
...
<div class="easyui-layout" fit="true">
 <div region="west" style="width: 180px;" title="组织机构" split="true">
  ...
  <div region="center" style="padding:0px;border:0px">
      <t:datagrid name="hmStaffList3" onLoadSuccess="gridLoadSuccess" onClick="doCancelSession" checkbox="true" pagination="true" fitColumns="false" title="" actionUrl="hmStaffController.do?datagridF&state=0" idField="id" fit="true" btnCls="bootstrap" queryMode="group">
   <t:dgCol title="主键"  field="id"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="创建人名称"  field="createName"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="创建人登录名称"  field="createBy"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="创建日期"  field="createDate"  formatter="yyyy-MM-dd"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
   <t:dgCol title="所属部门"  field="sysOrgCode"  hidden="true"  queryMode="single"  width="120"></t:dgCol>
          <t:dgCol title="工号" query="true"  field="lsJdNamesCode"  queryMode="single"  width="70"></t:dgCol>
          <t:dgCol title="姓名" query="true"  field="lsJdNames"  queryMode="single"  width="80"></t:dgCol>
 ...
 function saveSession() {
     var rowsData = $('#hmStaffList3').datagrid('getSelections');
     var ids = [];
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].lsJdNamesCode);
     }
     $.ajax({
         url : "hmStaffController.do?doSaveSession&r=lsJdNamesCodeS&ids="+ids,
         type : 'post',
         cache : false,
         data: null,
         success : function(data) {
