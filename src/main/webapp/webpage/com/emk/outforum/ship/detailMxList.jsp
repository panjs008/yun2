...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        <c:if test="${param.shipId eq null || param.shipId eq ''}">
            $('#addBtnSR').click();
        </c:if>
    });
...
<input id="orderMxListIDSR" type="hidden" name="orderMxListIDSR" value="${fn:length(emkShipDetailEntities)}"/>
<div class="table-c" style="overflow: auto; width: 100%;">
    <table id="mxtbSR" width="1800px"  border="0" cellspacing="0" cellpadding="0">
        ...
        <c:if test="${fn:length(emkShipDetailEntities)  > 0 }">
            <c:forEach items="${emkShipDetailEntities}" var="poVal" varStatus="status">
                <tr>
                    ...
                    <td align="center"><input  nullmsg="请输入发票编号！"  errormsg="请输入发票编号" value="${poVal.fpNum}" name="orderMxList[${status.index}].fpNum00" maxlength="100" type="text" value=""
                                              style="width: 86%;" ignore="ignore"></td>
                    <td align="center"><input nullmsg="请输入装箱单编号！"  errormsg="请输入装箱单编号" value="${poVal.boxNum}" name="orderMxList[${status.index}].boxNum00" maxlength="100" type="text" value=""
                                              style="width: 86%;" ignore="ignore"></td>
                </tr>
            </c:forEach>
        </c:if>
