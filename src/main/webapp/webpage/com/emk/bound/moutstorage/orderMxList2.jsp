...
<script type="text/javascript">
    ...
    $('#addBtn').bind('click', function(){
        ...
        $("#add_jeecgOrderProduct_table").find("[id='cytj00']").attr("id","cytj"+flag);
    ...
    function  getProduct(fIndex){
        ...
        if(pNum != null && pNum != ""){
            $.ajax({
                url : "emkMOutStorageController.do?getProductInfo&proNum="+pNum,
                type : 'post',
                cache : false,
                data: null,
    ...
    function setProNumNull(fIndex){
        $("#proNum" + fIndex).val("");
    }
    function  getProductNum(fIndex) {
        var pNum = $("#proNum" + fIndex).val();
        <c:if test="${param.node eq 'jhhsTask'}">
            if (pNum != null && pNum != "" && pNum.length == 18) {
                pNum = pNum.substr(pNum.indexOf('21')+2);
                $("#proNum" + fIndex).val(pNum);
                $.ajax({
                    url : "emkMOutStorageController.do?getProductInfo&inStorageId=${param.inStorageId}&proNum="+pNum,
                    type : 'post',
                    cache : false,
                    data: null,
                    success : function(data) {
                        ...
                        if (d.success) {
                            var obj = d.obj;
                            $("#proZnName"+fIndex).css("color","red");
                            $("#proNum"+fIndex).css("color","red");
                            $("#unit"+fIndex).css("color","red");
                            $("#standards"+fIndex).css("color","red");
                            $("#brand"+fIndex).css("color","red");
                            $("#scqy"+fIndex).css("color","red");
                            $("#lincese"+fIndex).css("color","red");
                            $("#zcNum"+fIndex).css("color","red");
                            $("#scDate"+fIndex).css("color","red");
                            $("#limitDate"+fIndex).css("color","red");
                            $("#betch"+fIndex).css("color","red");
                            $("#cytj"+fIndex).css("color","red");
                        }
                    }
                });
            }
...
<table style="width:100%;display: none;border: 1px;" cellpadding="0" cellspacing="2" border="0">
    ...
    <tr>
        ...
        <td align="center"><input id="betch00" nullmsg="请输入批号！"  errormsg="请输入批号" name="orderMxList[#index#].betch" maxlength="100" type="text" value=""
                                  style="width: 86%;" ignore="ignore"></td>
        <td align="center"><input id="cytj00" nullmsg="请输入储运条件！"  errormsg="请输入储运条件" name="orderMxList[#index#].cytj" maxlength="100" type="text" value=""
                                  style="width: 86%;" ignore="ignore"></td>
    </tr>
    </tbody>
...
<div style="display:none">
    ...
    <input id="cytj" name="cytj" type="text" />

    <input id="id" name="id" type="text" />
    <t:choose  hiddenName="id"  hiddenid="id" url="emkStorageController.do?proSelect" name="emkStorageList" width="820px" height="550px"
               icon="icon-search" title="选择产品" textname="id,proNum,proType,proTypeName,proZnName,standards,brand,unit,scqy,lincese,zcNum,scDate,limitDate,betch,cytj" isclear="true" isInit="true"></t:choose>
</div>
...
<div class="table-c" >
    <table id="mxtb" border="0" cellspacing="0" cellpadding="0">
        <tr bgcolor="#F8F8F8" style="height: 32px;" >
            ...
            <td align="center"  width="60">储运条件</td>
            ...
            <c:forEach items="${rkglMxList}" var="poVal" varStatus="status">
                <tr style="">
                    ...
                    <td align="center"><input  nullmsg="请输入产品名称！" id="proZnName${status.index}" ondblclick="productLook(${status.index});" onkeyup="getProduct(${status.index})"  errormsg="请输入产品名称" value="${poVal.proZnName}" name="orderMxList[${status.index}].proZnName" maxlength="100" type="text" value=""
                                               style="width: 92%;color:${poVal.flag eq '1' ? 'red':''}" ignore="ignore"></td>
                    <td align="center"><input  nullmsg="请输入产品编号！" id="proNum${status.index}" onclick="setProNumNull(${status.index})" onkeyup="getProductNum(${status.index})" errormsg="请输入产品编号" value="${poVal.proNum}" name="orderMxList[${status.index}].proNum" maxlength="100" type="text" value=""
                                               style="width: 92%;color:${poVal.flag eq '1' ? 'red':''}" ignore="ignore"></td>
                    <td align="center"><input  nullmsg="请输入规格！"  id="standards${status.index}" errormsg="请输入规格" value="${poVal.standards}" name="orderMxList[${status.index}].standards" maxlength="100" type="text" value=""
                                               style="width: 93%;color:${poVal.flag eq '1' ? 'red':''}" ignore="ignore"></td>
                    ...
                    <td align="center"><input nullmsg="请输入单位！" id="unit${status.index}" errormsg="请输入单位" value="${poVal.unit}" name="orderMxList[${status.index}].unit" maxlength="100" type="text" value=""
                                              style="width: 70%;color:${poVal.flag eq '1' ? 'red':''}" ignore="ignore"></td>
                    ...
                    <td align="center"><input  nullmsg="请输入生产日期！" id="scDate${status.index}" readonly onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})"  value="${poVal.scDate}" class="Wdate" errormsg="请输入生产日期" name="orderMxList[${status.index}].scDate" maxlength="100" type="text" value=""
                                               style="width: 86%;color:${poVal.flag eq '1' ? 'red':''}" ignore="ignore"></td>
                    ...
                    <td align="center"><input nullmsg="请输入批号！"  id="betch${status.index}" errormsg="请输入批号" value="${poVal.betch}" name="orderMxList[${status.index}].betch" maxlength="100" type="text" value=""
                                              style="width: 86%;color:${poVal.flag eq '1' ? 'red':''}" ignore="ignore"></td>
                    <td align="center"><input nullmsg="请输入储运条件！" id="cytj${status.index}" errormsg="请输入储运条件" value="${poVal.cytj}" name="orderMxList[${status.index}].cytj" maxlength="100" type="text" value=""
                                              style="width: 86%;color:${poVal.flag eq '1' ? 'red':''}" ignore="ignore"></td>
                </tr>
...
<script type="text/javascript">
    ...
    function returnToVal(){
        ...
        for(var i = 0 ; i < a2.length ; i++){
            ...
            $("#cytj"+(flag-a11.length+i+1)).val(a11[i]);
        }
