...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMOutStorageList" checkbox="false" pagination="true" fitColumns="true" title="" sortOrder="desc" sortName="kdDate"  actionUrl="emkMOutStorageController.do?datagrid&type=2" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
      ...
      <t:dgToolBar title="录入" icon="fa fa-plus" operationCode="add" url="emkMOutStorageController.do?goAdd&type=2&winTitle=添加包装辅料出库申请单" funname="add" height="600" width="1200"></t:dgToolBar>
      <t:dgToolBar title="编辑" icon="fa fa-edit" operationCode="edit" url="emkMOutStorageController.do?goUpdate&type=2&winTitle=编辑包装辅料出库申请单" funname="update" height="600" width="1200"></t:dgToolBar>
      <t:dgToolBar title="查看" icon="fa fa-search" operationCode="look" url="emkMOutStorageController.do?goUpdate&type=2&winTitle=查看包装辅料出库申请单" funname="detail" height="600" width="1200"></t:dgToolBar>
 ...
 function doPrintPDF() {
     ...
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要导出PDF的包装辅料出库申请单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定导出PDF的包装辅料出库申请单?', function(r) {
         if (r) {
 ...
 function doSubmitV() {
     ...
     var ids = [];
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要提交的包装辅料出库申请单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定提交包装辅料出库申请单?', function(r) {
         if (r) {
             $.ajax({
