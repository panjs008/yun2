...
<head>
	...
	<script src="${webRoot}/context/cusForCol3.js"></script>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("#detailId").load("emkMOutStorageController.do?orderMxList&proOrderId=${emkMOutStoragePage.id}");
					...
					<c:if test="${headlist.required eq '0'}">
						<c:choose>
							...
							<c:when test="${headlist.column_type eq '7' && headlist.code eq 'a01a07a06'}">
								<t:dictSelect id="${headlist.code}" field="${headlist.code}" typeGroupCode="paytype"  defaultVal="" hasLabel="false" title="付款方式"></t:dictSelect>
							</c:when>
							<c:otherwise>
							</c:otherwise>
						</c:choose>
