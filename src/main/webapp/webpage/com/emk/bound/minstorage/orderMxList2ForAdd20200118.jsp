...
<script type="text/javascript">
    var flag = ${fn:length(emkProOrderDetailEntityList)};
    var currentFlag = ${fn:length(emkProOrderDetailEntityList)};
    var returnCount = ${fn:length(emkProOrderDetailEntityList)};
    ...
    function selectOrder(){
        ...
        $.dialog({
            content: 'url:emkProOrderController.do?listForCg',
            zIndex: getzIndex(),
            title : '销售订单',
            cache:false,
            lock : true,
            ...
            cancelVal: '确定',
            cancel: function(){
                $("#detailId").load("emkMInStorageController.do?orderMxList2ForAdd&inStorageId=${emkMInStoragePage.id}");
                //$("#detailId").css("height",height-headTb-footTb);
            },
        });
//        createdetailwindow("销售订单", "emkProOrderController.do?listForCg", 1230, height);
    }
   ...
   function refreshT(){
    ...
    function addTr(){
        ...
        /*$("#add_jeecgOrderProduct_table").find("[id='proZnName00']").attr("id","proZnName"+flag);
         ...
         $("#add_jeecgOrderProduct_table").find("[id='costPrice00']").attr("id","costPrice"+flag);*/
    ...
    <tr>
        ...
        <%-- <td align="center"><input id="proZnName00" nullmsg="请输入产品名称！"  errormsg="请输入产品名称"  name="orderMxList[#index#].proZnName" maxlength="100" type="text" value=""
         ...
         <td align="center"><input id="unit00" nullmsg="请输入单位！"  errormsg="请输入单位" name="orderMxList[#index#].unit" maxlength="100" type="text" value=""
                                   style="width: 70%;border:0;" ignore="ignore"></td>
         <td align="center"><input id="costPrice00" nullmsg="请输入进价！"  errormsg="请输入进价" name="orderMxList[#index#].costPrice" maxlength="100" type="text" value=""
                                   style="width: 70%;border:0;" ignore="ignore"></td>--%>
        <c:forEach var="list" items="${categoryEntities}" varStatus="status">
            <td align="center" >
...
<div style="padding: 3px; height: 29px; width:100%;margin-bottom:4px;margin-top: 2px; " class="datagrid-toolbar"><a id="addBtn" href="#" title="新增">新增</a><a id="copyBtn" href="#" title="复制行">复制行</a><a id="delBtn" href="#" title="删除">删除</a> <a id="saleBtn" href="#" title="销售订单">销售订单</a> <a id="setBtn" href="#" title="表头列配置">表头列</a>  <a id="setBtn2" href="#" title="明细列配置">列配置</a></div>
<%--<table border="0" cellpadding="2" cellspacing="0" id="jeecgOrderProduct_table">--%>
<input id="orderMxListID" type="hidden" name="orderMxListID" value="${fn:length(emkProOrderDetailEntityList)}"/>
<div class="table-c" >
    <table id="mxtb" border="0" cellspacing="0" cellpadding="0">
        ...
        <c:if test="${fn:length(emkProOrderDetailEntityList)  > 0 }">
            <c:set var="sl" value="0"/>
            <c:set var="zj" value="0"/>
            <c:forEach items="${emkProOrderDetailEntityList}" var="poVal" varStatus="status">
                <tr>
                    ...
                    <td align="center" width="35">${status.index+1}<input style="width: 25px;" type="hidden" name="orderMxList[${status.index}].id" value="${poVal.id}"/></td>
                        <%-- <td align="center"><input  nullmsg="请输入产品名称！" id="proZnName${status.index}" onkeyup="getProduct(${status.index})" ondblclick="productLook(${status.index});"  errormsg="请输入产品名称" value="${poVal.proZnName}" name="orderMxList[${status.index}].proZnName" maxlength="100" type="text" value=""
                                                    style="width: 92%;border:0;" ignore="ignore"></td>
                         ...
                         <td align="center"><input  nullmsg="请输入规格！"  id="standards${status.index}" errormsg="请输入规格" value="${poVal.standards}" name="orderMxList[${status.index}].standards" maxlength="100" type="text" value=""
                                                    style="width: 93%;border:0;" ignore="ignore"></td>
                         ...
                         <td align="center"><input nullmsg="请输入单位！"  id="unit${status.index}"  errormsg="请输入单位" value="${poVal.unit}" name="orderMxList[${status.index}].unit" maxlength="100" type="text" value=""
                                                   style="width: 70%;border:0;" ignore="ignore"></td>
                         <td align="center"><input nullmsg="请输入进价！"  id="costPrice${status.index}" value="${poVal.costPrice}" onkeyup="setMoney(${status.index});" errormsg="请输入进价" value="${poVal.costPrice}" name="orderMxList[${status.index}].costPrice" maxlength="100" type="text" value=""
                                                   style="width: 70%;border:0;" ignore="ignore"></td>--%>
