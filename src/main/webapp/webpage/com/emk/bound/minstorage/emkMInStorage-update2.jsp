...
<body>
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="emkMInStorageController.do?doUpdateForOrder" tiptype="1">
			...
			<c:if test="${status.index % 3 ==0}">
				...
				<td class="value"  colspan="${fn:length(headcategoryEntities)-1 eq status.index && fn:length(headcategoryEntities) % 3 !=0 ? 5:1}" >
					...
					<c:if test="${headlist.required ne '0'}">
						<c:choose>
							<c:when test="${headlist.column_type eq '0'  || headlist.column_type eq '1'}">
								<input id="${headlist.code}" name="${headlist.code}" type="text"  ignore="ignore" value="${emkMInStoragePage[headlist.code]}" style="width: 150px"  class="inputxt" >
							</c:when>
							<c:when test="${headlist.column_type eq '2'}">
			...
			<c:if test="${status.index % 3 !=0}">
				...
				<td class="value">
					...
					<c:if test="${headlist.required eq '0'}">
						<c:choose>
							...
							<c:when test="${headlist.column_type eq '5' && headlist.code eq 'a01a03a11'}">
								...
								<input id="${headlist.code}" name="${headlist.code}" value="${emkMInStoragePage[headlist.code]}" type="hidden" value="${(ROLE.rolecode eq 'admin' || ROLE.rolecode eq 'useradmin') ? '':CUR_USER.storageId}" style="width: 150px" class="inputxt"  ignore="ignore" />
								<input id="storageName" name="storageName" type="hidden" value="${(ROLE.rolecode eq 'admin' || ROLE.rolecode eq 'useradmin') ? '':CUR_USER.storageName}"/>
					...
					<c:if test="${headlist.required ne '0'}">
						<c:choose>
							<c:when test="${headlist.column_type eq '0'  || headlist.column_type eq '1'}">
								<input id="${headlist.code}" name="${headlist.code}" type="text"  style="width: 150px" value="${emkMInStoragePage[headlist.code]}" ignore="ignore"  class="inputxt"  >
							</c:when>
							<c:when test="${headlist.column_type eq '2'}">
							...
							<c:when test="${headlist.column_type eq '5' && headlist.code eq 'a01a03a06'}">
								...
								<input id="${headlist.code}" name="${headlist.code}" value="${emkMInStoragePage[headlist.code]}"  type="hidden" style="width: 150px" class="inputxt"  ignore="ignore" />
								<input id="gys" name="gys" type="hidden"  value="${emkMInStoragePage.gys }"  style="width: 150px" class="inputxt"  ignore="ignore" />
								<%--<input id="preBill" name="preBill" type="hidden"  value="${emkMInStoragePage.preBill }"  style="width: 150px" class="inputxt"  ignore="ignore" />--%>
							</c:when>
							<c:when test="${headlist.column_type eq '5' && headlist.code eq 'a01a03a11'}">
	...
	<table id="footTb" style="width: 100%;position:fixed; left:0; bottom:0;" cellpadding="0" cellspacing="1" class="formtable">
		...
		<tr>
			...
			<td class="value" >
				<input id="bcqkMoney" name="bcqkMoney"  datatype="d" value="${emkMInStoragePage.bcqkMoney}"  style="width: 150px" class="inputxt"  ignore="ignore" />
				<span class="Validform_checktip"></span>
			...
			<td class="value" >
				<input id="qcqkMoney" name="qcqkMoney"  datatype="d" value="${emkMInStoragePage.qcqkMoney}"  style="width: 150px" class="inputxt"  ignore="ignore" />
				<span class="Validform_checktip"></span>
				<label class="Validform_label" style="display: none;">本次欠款</label>
			</td>
			<td align="right" >
			...
			<td class="value" colspan="3">
				<input id="ljqkMoney" name="ljqkMoney"  datatype="d" value="${emkMInStoragePage.ljqkMoney}"  style="width: 150px" class="inputxt"  ignore="ignore" />
				<span class="Validform_checktip"></span>
...
<script src="${webRoot}/context/gysForColUpt.js"></script>
<script>
