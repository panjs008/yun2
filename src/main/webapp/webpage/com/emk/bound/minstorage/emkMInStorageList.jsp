...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMInStorageList" checkbox="true" singleSelect="true" pagination="true" pageSize="20" fitColumns="true" sortOrder="desc" sortName="a01a03a07" title="查询条件" actionUrl="emkMInStorageController.do?datagrid&flag=0" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
    ...
    <%--  <t:dgCol title="操作" field="opt"   width="100"></t:dgCol>
      ...
      <c:forEach var="list" items="${headcategoryEntities}" varStatus="status">
          <c:if test="${list.code ne 'a01a03a06' && list.code ne 'a01a03a11' && list.code ne 'a01a03a08'}">
              <t:dgCol title="${list.name}"  field="${list.code}" queryMode="single" dictionary="${list.code eq 'a01a03a09' ? 'paytype':''}" query="${list.queryed eq 0 ? 'true':'false'}"  width="${fn:contains(list.name,'单号') ? '130':( list.column_type eq '4' ? '210':(list.column_type eq '0' || list.column_type eq '2') ? '50':'100')}"></t:dgCol>
          </c:if>
      ...
      <%--<t:dgFunOpt funname="goToProcess(id,createBy,processName,state)" title="流程进度" urlclass="ace_button"  urlStyle="background-color:#ec4758;" urlfont="fa-tasks"></t:dgFunOpt>--%>
       ...
       <t:dgToolBar title="录入" icon="fa fa-plus" operationCode="add"  url="emkMInStorageController.do?goAdd&type=0&winTitle=添加采购入库单" funname="add" height="600" width="1200"></t:dgToolBar>
       <t:dgToolBar title="编辑" icon="fa fa-edit" operationCode="edit" url="emkMInStorageController.do?goUpdate&type=0&winTitle=编辑采购入库单" funname="update" height="600" width="1200"></t:dgToolBar>
      <t:dgToolBar title="查看" icon="fa fa-search" operationCode="look"  url="emkMInStorageController.do?goUpdate&type=0&winTitle=查看采购入库单" funname="detail" height="600" width="1200"></t:dgToolBar>
      ...
      <t:dgToolBar title="收货"  icon="fa fa-check-square-o" operationCode="recsubmit" url="emkMInStorageController.do?goRecevie&winTitle=采购入库单收货" funname="update" height="200" width="400"></t:dgToolBar>

      <t:dgToolBar title="删除"   icon="fa fa-remove" operationCode="delete" url="emkMInStorageController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
