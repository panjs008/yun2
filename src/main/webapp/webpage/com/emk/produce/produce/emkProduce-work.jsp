...
<div id="orderDiv" title="流程图" style="width:100%;height:310px;overflow:hidden" >
	<iframe scrolling="no" id="processFrm" frameborder="0" style="margin-top: -8px;margin-left:0px;"  src="flowController.do?showProcess&id=${param.id}&tableName=emk_produce_schedule" width="100%" height="100%"></iframe>
</div>
<div id="timeDiv" title="流程时间轴" style="width:100%;height:300px;overflow:hidden" >
	<iframe scrolling="no" id="timeFrm" frameborder="0"  src="emkProduceController.do?goTime&id=${param.id}" width="100%" height="100%"></iframe>
</div>

<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="emkProduceController.do?doSubmit"  tiptype="1" >
	...
	<table id="adviceTb" style="width: 100%;margin-top:2px;margin-bottom: 4px" cellpadding="0" cellspacing="1" class="formtable">
		<c:if test="${(emkProducePage.state eq 1 or emkProducePage.state eq 70 or emkProducePage.state eq 75 or emkProducePage.state eq 77 or
							emkProducePage.state eq 82  or emkProducePage.state eq 83 or emkProducePage.state eq 85) && ROLE.rolecode eq 'ywgdy'
		 				|| emkProducePage.state eq 79 && ROLE.rolecode eq 'cwy'
		 				|| (emkProducePage.state eq 3 or emkProducePage.state eq 27 or emkProducePage.state eq 84) && ROLE.rolecode eq 'cw'
		 				|| (emkProducePage.state eq 66 or emkProducePage.state eq 67) && ROLE.rolecode eq 'zjy'
		 				|| (emkProducePage.state eq 79 or emkProducePage.state eq 4) && ROLE.rolecode eq 'ywjl' && param.node eq 'ywjlzfhTask'
		 				|| emkProducePage.state eq 86 && ROLE.rolecode eq 'cky'
		 				|| emkProducePage.state eq 25 && ROLE.rolecode eq 'zjl' && param.node eq 'zjlpzTask'}">
			<tr>
				<td align="right">
			...
			<c:if test="${emkProducePage.state eq 61 && ROLE.rolecode eq 'zjjl'  && param.node eq 'fpyhyTask'}">
				<tr id="chooseUser">
			...
			<c:if test="${emkProducePage.state eq 67 && ROLE.rolecode eq 'zjy' && param.node eq 'bgTask'}">
				<tr>
					<td align="right">
						<label class="Validform_label">
							测试报告:
