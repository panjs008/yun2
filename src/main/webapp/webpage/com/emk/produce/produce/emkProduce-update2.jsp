...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="emkProduceController.do?doUpdate" tiptype="1">
	...
	<table style="width: 100%;margin-top: 5px;" cellpadding="0" cellspacing="1" class="formtable">
		<c:if test="${param.node eq 'cgscjdbTask' || param.node eq 'cgTask' || param.node eq 'ylblTask' || param.node eq 'ranTask'
							|| param.node eq 'caiTask' || param.node eq 'fengTask' || param.node eq 'bzTask'}">
			<tr>
				<td align="right" >
			...
			<td class="value" colspan="3">
				<select id="businesser"  name="businesser" datatype="*">
					...
					<c:forEach items="${ywyList}" var="ywy">
						<option value='${ywy.userName}' ${ywy.userName eq emkProducePage.businesser ? 'selected':''}>${ywy.realName}</option>
					</c:forEach>
				</select>
			...
			<td class="value" colspan="3">
				<select id="tracer"  name="tracer" datatype="*">
					...
					<c:forEach items="${ywgdyList}" var="ywgdy">
						<option value='${ywgdy.userName}' ${ywgdy.userName eq emkProducePage.tracer ? 'selected':''}>${ywgdy.realName}</option>
					</c:forEach>
				</select>
			...
			<td class="value" colspan="3">
				<select id="developer"  name="developer" datatype="*">
					...
					<c:forEach items="${scgdyList}" var="scgdy">
						<option value='${scgdy.userName}'  ${scgdy.userName eq emkProducePage.developer ? 'selected':''}>${scgdy.realName}</option>
					</c:forEach>
				</select>
