...
<div id="orderDiv" title="流程图" style="width:100%;height:280px;overflow:hidden" >
	<iframe scrolling="no" id="processFrm" frameborder="0" style="margin-top: -8px;margin-left:0px;"  src="flowController.do?showProcess&id=${param.id}&tableName=emk_test" width="100%" height="100%"></iframe>
</div>
<div id="timeDiv" title="流程时间轴" style="width:100%;height:300px;overflow:hidden" >
	<iframe scrolling="no" id="timeFrm" frameborder="0"  src="emkTestController.do?goTime&id=${param.id}" width="100%" height="100%"></iframe>
</div>
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="emkTestController.do?doSubmit"  tiptype="1" >
	...
	<table id="adviceTb" style="width: 100%;margin-top:2px;margin-bottom: 4px" cellpadding="0" cellspacing="1" class="formtable">
		<c:if test="${emkTestPage.state eq 1 && ROLE.rolecode eq 'ywgdy'
		 				|| (emkTestPage.state eq 61 || emkTestPage.state eq 65) && ROLE.rolecode eq 'scgdy'
		 				|| emkTestPage.state eq 62 && ROLE.rolecode eq 'tester'}">
			<tr>
				<td align="right">
			...
			<c:if test="${emkTestPage.state eq 61 && ROLE.rolecode eq 'scgdy'  && param.node eq 'csTask'}">
				<tr id="chooseUser">
			...
			<c:if test="${emkTestPage.state eq 65 && ROLE.rolecode eq 'scgdy' && param.node eq 'bgTask'}">
				<tr>
					<td align="right">
						<label class="Validform_label">
							测试报告:
