...
<div class="easyui-layout" fit="true">
    <div region="center" style="padding:0px;border:0px">
        <t:datagrid name="emkMaterialContractList" checkbox="false" pagination="true" fitColumns="true" title="" actionUrl="emkMaterialContractController.do?datagrid&type=2" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
            ...
            <t:dgCol title="包装辅料开发费付款申请单号"  query="true"  field="payNo" queryMode="single" width="200"></t:dgCol>
...
<script type="text/javascript">
    ...
    function goToProcess(id,createBy,processName,state){
        ...
        if(state == '0'){
            createwindow("流程进度--当前环节：包装辅料开发费付款申请单", "flowController.do?goProcess&state="+state+"&node="+node+"&processUrl=com/emk/bill/materialcontract/emkMaterialContract-process&id=" + id, 1300, height);
        }else {
    ...
    function doSubmitV() {
        ...
        if (!rowsData || rowsData.length == 0) {
            tip('请选择需要提交的包装辅料开发费付款申请单');
            return;
        }
        for ( var i = 0; i < rowsData.length; i++) {
            ids.push(rowsData[i].id);
        }
        $.dialog.confirm('您是否确定提交包装辅料开发费付款申请单?', function(r) {
            if (r) {
                $.ajax({
