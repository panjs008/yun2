				...
				<li>
					<div class="liwrap">
						...
						<div class="liright">
							...
							<div class="histt"><a style="font-size: 12px;"><c:if test="${task.TASK_DEF_KEY_ eq 'htTask'}">【${task.workname}】发起购销合同申请单，合同单号：${emkContract.htNum}</c:if>
								<c:if test="${task.TASK_DEF_KEY_ eq 'checkTask'}">审核人【${emkContract.leader}】，处理意见：${emkContract.leadAdvice}</c:if>
							</a></div>
						</div>
					</div>
				</li>
