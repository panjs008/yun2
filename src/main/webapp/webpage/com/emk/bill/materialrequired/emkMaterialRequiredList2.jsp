...
<div id="main_list" class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMaterialRequiredList" checkbox="false" pagination="true" fitColumns="false" title="" actionUrl="emkMaterialRequiredController.do?datagrid&type=1" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
...
<%--<div data-options="region:'east',
 ...
 function goToProcess(id,createBy,processName,state){
     ...
     if(createBy == "${CUR_USER.userName}" || ${CUR_USER.userName eq 'admin'}){
         if(state == '0'){
             createwindow("流程进度--当前环节：缝制采购需求单", "flowController.do?goProcess&node="+node+"&processUrl=com/emk/bill/materialrequired/emkMaterialRequired-process&id=" + id, 1300, height);
         }else {
 ...
 function doPrintPDF() {
     ...
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要导出PDF的缝制采购需求单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定导出PDF的缝制采购需求单?', function(r) {
         if (r) {
 ...
 function doSubmitV() {
     ...
     $.dialog.confirm('您是否确定提交缝制采购需求单?', function(r) {
         if (r) {
             $.ajax({
 ...
 function queryDetail2(id,proName){
     ...
     var title = "缝制面料清单："+proName+","+rowsData[0].cusName;
