...
<div id="main_list" class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMaterialRequiredList" checkbox="false" pagination="true" fitColumns="false" title="" actionUrl="emkMaterialRequiredController.do?datagrid&type=2" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
      ...
      <t:dgToolBar title="编辑" icon="fa fa-edit" operationCode="edit" url="emkMaterialRequiredController.do?goUpdate&type=2&winTitle=编辑包装采购需求单" funname="update" height="600" width="1200"></t:dgToolBar>
...
<%--<div data-options="region:'east',
 ...
 function goToProcess(id,createBy,processName,state){
     ...
     if(createBy == "${CUR_USER.userName}" || ${CUR_USER.userName eq 'admin'}){
         if(state == '0'){
             createwindow("流程进度--当前环节：包装采购需求单", "flowController.do?goProcess&node="+node+"&processUrl=com/emk/bill/materialrequired/emkMaterialRequired-process&id=" + id, 1300, height);
         }else {
 ...
 function doSubmitV() {
     ...
     $.dialog.confirm('您是否确定提交包装采购需求单?', function(r) {
         if (r) {
             $.ajax({
