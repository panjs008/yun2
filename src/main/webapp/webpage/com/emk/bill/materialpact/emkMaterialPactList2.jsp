...
<div id="main_list" class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMaterialPactList" checkbox="true" pagination="true" fitColumns="false" title="" actionUrl="emkMaterialPactController.do?datagrid&type=1&flag=0" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
   ...
   <%--<t:dgCol title="操作" field="opt" frozenColumn="true"  width="100"></t:dgCol>--%>
    <%--  <t:dgCol title="采购合同编号"  field="materialNo" queryMode="single" query="true"  width="120"></t:dgCol>
      ...
      <t:dgToolBar title="录入" icon="fa fa-plus" operationCode="add" url="emkMaterialPactController.do?goAdd&type=1&winTitle=录入缝制预采购合同单" funname="add" height="600" width="1350"></t:dgToolBar>
      <t:dgToolBar title="查看" icon="fa fa-search" operationCode="look" url="emkMaterialPactController.do?goUpdate&type=1&winTitle=查看缝制预采购合同单" funname="detail" height="600" width="1350"></t:dgToolBar>
      <t:dgToolBar title="编辑" icon="fa fa-edit" operationCode="edit" url="emkMaterialPactController.do?goUpdate&type=1&winTitle=编辑缝制预采购合同单" funname="update" height="600" width="1350"></t:dgToolBar>
 ...
 function goToProcess(id,createBy,createName){
     ...
     $.ajax({
         url: "flowController.do?getCurrentProcess&tableName=emk_material_pact&title=缝制预采购合同单&id=" + id,
         type: 'post',
         cache: false,
         data: null,
 ...
 function doPrintPDF() {
     ...
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要导出PDF的缝制预采购合同单生成合同单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定导出PDF的缝制预采购合同单生成合同单?', function(r) {
         if (r) {
 ...
 function doSubmitV() {
     ...
     var ids = [];
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要提交的缝制预采购合同单');
         return;
     }
     for (var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定提交缝制预采购合同单?', function (r) {
         if (r) {
             $.ajax({
 ...
 function doGenerateV() {
     ...
     $.dialog.confirm('您是否确定提交缝制预采购合同单?', function (r) {
         if (r) {
             $.ajax({
                 url: "emkMaterialPactController.do?doGenerate&type=1&ids=" + ids,
                 type: 'post',
                 cache: false,
                 data: null,
                 success: function (data) {
