...
<div id="main_list" class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMaterialPactList" checkbox="false" pagination="true" fitColumns="false" title="" actionUrl="emkMaterialPactController.do?datagrid&type=0&flag=1" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
   ...
   <%--<t:dgCol title="款式大类"  field="proTypeName"  queryMode="single"  width="70"></t:dgCol>--%>
      ...
      <%--<t:dgToolBar title="录入" icon="fa fa-plus" operationCode="add" url="emkMaterialPactController.do?goAdd&type=0&winTitle=录入原料采购合同单" funname="add" height="600" width="1000"></t:dgToolBar>--%>
      ...
      <t:dgToolBar title="编辑" icon="fa fa-edit" operationCode="edit" url="emkMaterialPactController.do?goUpdate2&type=0&winTitle=编辑原料采购合同单" funname="update" height="600" width="1350"></t:dgToolBar>
 ...
 function goToProcess(id,createBy){
     ...
     $.ajax({
         url: "flowController.do?getCurrentProcess&tableName=emk_material_pact&title=原料采购合同单&id=" + id,
         type: 'post',
         cache: false,
         data: null,
 ...
 function doPrintPDF() {
     ...
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要导出PDF的原料采购合同单生成合同单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定导出PDF的原料采购合同单生成合同单?', function(r) {
         if (r) {
 ...
 function doSubmitV() {
     ...
     $.dialog.confirm('您是否确定提交原料采购合同单?', function (r) {
         if (r) {
             $.ajax({
