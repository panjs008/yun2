...
<div id="main_list" class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMaterialPactList" checkbox="true" pagination="true" fitColumns="false" title="" actionUrl="emkMaterialPactController.do?datagrid&type=0&flag=0" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
...
<%--<div data-options="region:'east',
 ...
 function queryDetail2(id,proName,state){
      ...
      $('#proDetialListpanel').panel("refresh", "emkSampleDetailController.do?list&state="+state+"&&sampleType=order&sampleId=" + id);
 }
 ...
 function goToProcess(id,createBy,createName){
     ...
     $.ajax({
         url: "flowController.do?getCurrentProcess&tableName=emk_material_pact&title=原料预采购合同单&id=" + id,
         type: 'post',
         cache: false,
         data: null,
 ...
 function doPrintPDF() {
     ...
     var ids = [];
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要导出PDF的原料预采购合同单生成合同单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定导出PDF的原料预采购合同单生成合同单?', function(r) {
         if (r) {
 ...
 function doGenerateV() {
     ...
     $.dialog.confirm('您是否确定提交原料预采购合同单?', function (r) {
         if (r) {
             $.ajax({
                 url: "emkMaterialPactController.do?doGenerate&type=0&ids=" + ids,
                 type: 'post',
                 cache: false,
                 data: null,
                 success: function (data) {
