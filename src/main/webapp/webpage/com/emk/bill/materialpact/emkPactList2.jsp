...
<div id="main_list" class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMaterialPactList" checkbox="false" pagination="true" fitColumns="false" title="" actionUrl="emkMaterialPactController.do?datagrid&type=1&flag=1" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
   ...
   <%--<t:dgToolBar title="录入" icon="fa fa-plus" operationCode="add" url="emkMaterialPactController.do?goAdd&type=1&winTitle=录入缝制预采购合同单" funname="add" height="600" width="1000"></t:dgToolBar>--%>
 ...
 function queryDetail2(id,proName){
      ...
      var title = "缝制辅料清单："+proName+","+rowsData[0].cusName;
 ...
 function goToProcess(id,createBy){
     ...
     $.ajax({
         url: "flowController.do?getCurrentProcess&tableName=emk_material_pact&title=缝制采购合同单&id=" + id,
         type: 'post',
         cache: false,
         data: null,
 ...
 function doPrintPDF() {
     ...
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要导出PDF的缝制采购合同单生成合同单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定导出PDF的缝制采购合同单生成合同单?', function(r) {
         if (r) {
 ...
 function doSubmitV() {
     ...
     $.dialog.confirm('您是否确定提交缝制采购合同单?', function (r) {
         if (r) {
             $.ajax({
