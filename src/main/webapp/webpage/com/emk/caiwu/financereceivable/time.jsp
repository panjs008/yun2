				...
				<li>
					<div class="liwrap">
						...
						<div class="liright">
							...
							<div class="histt"><a style="font-size: 12px;"><c:if test="${task.TASK_DEF_KEY_ eq 'ysTask'}">【${task.workname}】发起应收通知单，应收通知单号：${emkFinanceReceivable.receiveNum}</c:if>
								<c:if test="${task.TASK_DEF_KEY_ eq 'checkTask'}">审核人【${emkFinanceReceivable.leader}】，处理意见：${emkFinanceReceivable.leadAdvice}</c:if>
							</a></div>
						</div>
					</div>
				</li>
