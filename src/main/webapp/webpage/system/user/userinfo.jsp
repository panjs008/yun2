...
<t:formvalid formid="formobj" layout="div" dialog="true">
	<fieldset class="step">
		<div class="form"><label class="form"> 角色: </label> ${ROLE.rolename}</div>
		<c:if test="${custom.cusName eq null}">
			<div class="form"><label class="form"> 组织机构: </label> ${CUR_USER.currentDepart.departname}</div></c:if>
		<c:if test="${custom.cusName ne null}">
			<div class="form"><label class="form"> 服务商名称: </label> ${custom.cusName}</div></c:if>
		...
		<div class="form"><label class="form"> 办公电话: </label> ${user.officePhone}</div>
